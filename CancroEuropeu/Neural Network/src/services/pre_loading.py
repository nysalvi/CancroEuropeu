...
class PreLoading:
    ...
    def dataInstances(self, train_data, dev_data, test_data):
        print("Train instances: {} ({})".format(
            len(train_data),
            pd.Series([y for _, y in train_data]).value_counts().to_dict()
        ))
        print("Dev instances: {} ({})".format(
            len(dev_data),
            pd.Series([y for _, y in dev_data]).value_counts().to_dict()
        ))
    ...
    def dataLoaders(self, batch_size, train_data, dev_data, test_data) -> any:
        train_loader = data_utils.DataLoader(train_data, batch_size=batch_size, shuffle=True)
        dev_loader = data_utils.DataLoader(dev_data, batch_size=batch_size, shuffle=True)
        ...
        dev_images, dev_labels = next(dev_iter)
